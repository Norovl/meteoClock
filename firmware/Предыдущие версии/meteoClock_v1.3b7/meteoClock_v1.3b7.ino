...
byte MIN_ONDATA = 1 + 2 + 4 + 8 + 16 + 32 + 64 + 128; // минимальные показания графиков исходя из накопленных фактических (но в пределах лимитов) данных вместо указанных пределов, 0 - использовать фиксированные пределы (с)НР
...
boolean saveSet = false; // переменная для выбора статуса сохранения (с)НР
...
// Ь
uint8_t BM[8] = {0b10000,  0b10000,  0b10000,  0b11111,  0b10001,  0b10001,  0b11111,  0b00000};
// Й
...
void setup() {
  ...
  if (DISPLAY_TYPE == 1) {
    ...
    //drawSensors();    // замена drawClock  (с)НР
    drawData();
  }
  readSensors();
  drawSensors();
