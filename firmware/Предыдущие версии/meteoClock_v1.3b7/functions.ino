void modesTick() {
  ...
  // Serial.print(mode);Serial.print(" ");Serial.println(saveSet); // Отладка (с)НР

  if (button.isHolded()) {
    if (mode == 0) bigDig = !bigDig;
    if (mode == 255) {             // Сохранение настроек (с)НР
      if (saveSet) {
        if (EEPROM.read(2) != MAX_ONDATA) EEPROM.write(2, MAX_ONDATA);
        if (EEPROM.read(3) != MIN_ONDATA) EEPROM.write(3, MIN_ONDATA);
        if (EEPROM.read(4) != mode0scr) EEPROM.write(4, mode0scr);
        if (EEPROM.read(5) != bigDig) EEPROM.write(5, bigDig);
        if (EEPROM.read(0) == 255) EEPROM.write(0, 210);
  ...
  if (changeFlag) {
    if (mode == 255) {              // --------------------- показать вопрос "Сохранить настройки?" 
      lcd.clear();
      lcd.createChar(4, II);
      lcd.createChar(1, BM);
      lcd.createChar(2, IY);
      lcd.createChar(3, DD);
      lcd.setCursor(0, 0);
      lcd.print("COXPAH" + String(char(4)) + "T" + String(char(1)) + " HACTPO" + String(char(2)) + "K" + String(char(4)) + "?");
      lcd.setCursor(0, 1);
      lcd.print(" " + String(char(3)) + "A " + String(char(0b01111110)) + "HET");
    }
    else if (mode == 0) {
      lcd.clear();
      loadClock();
...
void clockTick() {
  ...
  if (dotFlag) {          // каждую секунду пересчёт времени
    ...
    if (mins > 59) {      // каждый час
      ...
      mins = now.minute();
      hrs = now.hour();
      //    if (mode == 0 && mode0scr == 0) drawClock(hrs, mins, 0, 0, 1);
