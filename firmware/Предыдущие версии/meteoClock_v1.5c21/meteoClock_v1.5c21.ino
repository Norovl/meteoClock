...
#define PRESSURE 1        // 0 - график давления, 1 - график прогноза дождя (вместо давления). Не забудь поправить пределы графика
...
byte MAX_ONDATA = 1 + 2 + 4 + 8 + 16 + 32 + 64 + 128; // при отсутствии сохранения в EEPROM: максимальные показания графиков исходя из накопленных фактических (но в пределах лимитов) данных вместо указанных пределов, 0 - использовать фиксированные пределы (с)НР
byte VIS_ONDATA = 1 + 2 + 4 + 8 + 16 + 32 + 64 + 128; // при отсутствии сохранения в EEPROM: отображение показания графиков, 0 - Не отображать (с)НР
...
void drawdots(byte x, byte y, boolean state) {  // Точки и статус питания (с)НР
  ...
  if (mode0scr == 0 && (bigDig && DISPLAY_TYPE == 0 || DISPLAY_TYPE == 1)) {          // мигание большими точками только в нулевом режиме главного экрана (с)НР
    lcd.setCursor(x, y);
    if (bigDig && DISPLAY_TYPE == 1) lcd.setCursor(x, y + 2);
    lcd.write(code);
    lcd.setCursor(x, y + 1);
    lcd.write(code);
  }
...
void setLED1(byte color) {                    // цвет индикатора задается двумя битами на цвет (с)НР
  ...
  //  if (color & 3) analogWrite(LED_R, LED_ON + LED_ON * (LED_MODE * 2 - 1) / 3 * (3 - (color % 4)));
  //  else analogWrite(LED_R, LED_OFF);
  //  if (color & 12) analogWrite(LED_G, LED_ON + LED_ON * (LED_MODE * 2 - 1) / 3 * (3 - ((color >> 2) % 4)));
  //  else analogWrite(LED_G, LED_OFF);
  //  if (color & 48) analogWrite(LED_B, LED_ON + LED_ON * (LED_MODE * 2 - 1) / 3 * (3 - ((color >> 4) % 4)));
  //  else analogWrite(LED_B, LED_OFF);
}
...
void setLED() {
  ...
  // ниже задается цвет индикатора: красный, желтый, зеленый, синий (с)НР
  if ((dispCO2 >= maxCO2) && LEDType == 0 || (dispHum <= minHum) && LEDType == 1 || (dispTemp >= maxTemp) && LEDType == 2 || (dispRain <= -50) && LEDType == 3) setLED1(3);            // красный
  else if ((dispCO2 >= normCO2) && LEDType == 0 || (dispHum <= normHum) && LEDType == 1 || (dispTemp >= normTemp) && LEDType == 2 || (dispRain <= 0) && LEDType == 3) setLED1(3 + 8);  // желтый
  else if (LEDType == 0 || (dispHum <= maxHum) && LEDType == 1 || (dispTemp >= minTemp) && LEDType == 2 || (dispRain <= 50) && LEDType == 3) setLED1(12);                              // зеленый
  else setLED1(48);   // синий (если влажность превышает заданный максимум, температура ниже минимума, вероятность осадков выше максимума)
}
